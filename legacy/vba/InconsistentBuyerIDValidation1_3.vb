    ...
    ' ============================================================================
    ' INCONSISTENT BUYER ID VALIDATION v1.3
    ' ============================================================================
    ' This macro tests for inconsistent buyer identification codes across
    ...
    ' - Validates IDs using format and logic validation from BuyerIDValidation v5.6
    ...
    ' 6-Person Code, 7-Account Type, 8-Buyer ID Code, 9-Type of Buyer ID Code
    ' 10-Buyer First Name(s), 11-Buyer Surname(s), 12-Buyer DOB, 13-Buyer Gender
    ...
    Private Const COL_ID_VALUE As Long = 7           ' Buyer ID Code
    Private Const COL_ID_TYPE As Long = 8            ' Type of Buyer ID Code
    Private Const COL_FNAME As Long = 9              ' Buyer - First Name(s)
    Private Const COL_SNAME As Long = 10             ' Buyer - Surname(s)
    Private Const COL_DOB As Long = 11               ' Buyer - DOB
    Private Const COL_GENDER As Long = 12            ' Buyer - Gender
    ...
    Sub InconsistentBuyerIDValidation1_3()
    ...
    ErrorHandler:
        Call HandleError(Err.Number, Err.Description, "InconsistentBuyerIDValidation1_3")
        GoTo CleanupAndExit
    End Sub
    ...
    Private Sub ProcessInconsistentIDValidation(ByRef clientRecords() As ClientRecord, ByRef context As ValidationContext)
        ' Main logic for inconsistent buyer ID validation
    ...
    If Not originalTypeValid And client.OriginalID <> "" Then
        Dim allowedTypes As Variant
    ...
    Dim hasFormats As Boolean
    hasFormats = HasCountryFormats(client.PriorityCountryCode, context.FormatDict)
    
    ' For countries WITH formats, test against patterns
    ' For countries WITHOUT formats (ROW), accept CONCAT automatically
    Dim formatValid As Boolean
    If hasFormats Then
        formatValid = TestIDAgainstAllPatterns(concatID, client.PriorityCountryCode, "CONCAT", context.FormatDict, context.RegexEngine)
    Else
        ' Rest of world countries - no format to test against, so accept CONCAT
        formatValid = True
    End If
    
    If formatValid Then
        ' Validate logic as well (v5.6 enhancement) - only if we have DOB for logic validation
        Dim logicValid As Boolean
        logicValid = ValidateIDLogic(concatID, "CONCAT", client.PriorityCountryCode, Format(client.DateOfBirth, "yyyymmdd"), client.Gender)
        
        If logicValid Then
            ' Accept only if both format AND logic valid (or ROW with valid logic)
...
Exit Function
    ...
    Private Function IsCONCATEligible(ByRef client As ClientRecord, ByRef context As ValidationContext) As Boolean
        ' CONCAT is eligible if client has required data (DOB, names)
        ' AND either:
        ' 1. Country has a CONCAT format defined in ID Formats, OR
        ' 2. Country has NO formats defined at all (rest of world)
        
        Dim hasRequiredData As BooleanCa
        hasRequiredData = (client.DateOfBirth <> 0 And _
                          client.FirstName <> "" And _
                          client.Surname <> "")
        
        If Not hasRequiredData Then
            IsCONCATEligible = False
            Exit Function
        End If
        
        ' Check if country has CONCAT format defined
        Dim hasCONCATFormat As Boolean
        hasCONCATFormat = context.FormatDict.exists(client.PriorityCountryCode & ":CONCAT:1")
        
        ' Check if country has ANY formats defined
        Dim hasAnyFormat As Boolean
        hasAnyFormat = HasCountryFormats(client.PriorityCountryCode, context.FormatDict)
        
        ' Allow CONCAT if it has a format OR if country has no formats at all
        IsCONCATEligible = hasCONCATFormat Or Not hasAnyFormat
    End Function
...
Private Function RemoveNamePrefixes(surname As String) As String
    ...
    Private Sub ShowCompletionMessage(recordCount As Long, processingTime As Double)
        Dim msg As String
        msg = "Inconsistent Buyer ID Validation completed successfully!" & vbNewLine & vbNewLine
