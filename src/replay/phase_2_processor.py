...
from core.data import ClientErrorColumns, Phase2CombinedColumns, Phase2SingleColumns
...
def _read_rows_from_file(filepath: str, logger=None) -> List[List[str]]:
    ...
    if ext == ".xlsx":
        ...
        for excel_row in ws.iter_rows(values_only=True):
            rows.append(
                ["" if cell is None else str(cell).strip() for cell in excel_row]
            )
        wb.close()
        return rows
    else:
...
class IncidentFileIndex:
    ...
    def _build_transaction_index(self):
        ...
        if txn_ref_col is None:
            if self.logger:
                self.logger.error(
                    f"Transaction Reference column not found in {self.file_path}"
                )
            return
...
class Phase2Processor:
    ...
    def parse_replay_record(
        self, row: List[str], row_index: int, file_type: str, filename: str
    ) -> ReplayRecord:
        ...
        try:
            ...
            self.logger.error(
                f"Error parsing replay record at row {row_index + 1} in {filename}: {e}"
            )
            return ReplayRecord(
                record_type="phase2",
                incident_codes=[],
    ...
    def preload_and_index_incident_files(self):
        ...
        replay_files = [
            f
            for f in os.listdir(self.path_config.replay_input)
        ...
        for incident_code in incident_codes:
            ...
            if incident_file:
                ...
                self.logger.warning(
                    f"Incident file not found for code: {incident_code}"
                )

        self.logger.info(f"Successfully indexed {loaded_count} incident files")

    def lookup_transaction(
        self, record: ReplayRecord, incident_code: str
    ) -> LookupResult:
    ...
    def _create_lookup_result(
        self, row: List[str], column_mapper: IncidentColumnMapper
    ) -> LookupResult:
        ...
        try:
            ...
            if correction_value:
                ...
                if agree_value in ("Y", "P", ""):
                    ...
                    self.logger.debug(
                        f"Applying Correction (Agree={agree_value or 'empty'})"
                    )
                # If agree is N or F -> check Suggested Correction
                    ...
                    if suggested_correction_value:
                        ...
                        self.logger.debug(
                            f"Applying Suggested Correction (analyst disagreed)"
                        )
                    else:
                        self.logger.debug(
                            f"No correction (analyst disagreed, no suggestion provided)"
                        )
                else:
                ...
                if suggested_correction_value:
                    ...
                    correction_field = suggested_correction_field_value
                    self.logger.debug(
                        f"Applying Suggested Correction (no automated correction)"
                    )
                else:
            ...
            if re.search(r"\bre\s+accounts?\b", correction, re.IGNORECASE):
                self.logger.debug(
                    "RE Account correction detected, treating as No Change"
                )
                correction = "No Change"
                correction_field = "No Change"
    ...
    def process_replay_record(self, record: ReplayRecord) -> Tuple[str, str, str]:
        ...
        for incident_code in record.incident_codes:
            ...
            if result.found:
                ...
                self.logger.debug(
                    f"Found match for {record.transaction_reference} in {incident_code}"
                )
            else:
                self.logger.warning(
                    f"Transaction {record.transaction_reference} not found in {incident_code}"
                )
        ...
        if len(unique_corrections) > 1 or len(unique_fields) > 1:
            ...
            self.logger.warning(
                f"Inconsistent corrections for {record.transaction_reference}: {unique_corrections}"
            )
        ...
        try:
            ...
            for batch_start in range(0, total_rows, batch_size):
                ...
                for i in range(batch_start, batch_end):
                    ...
                    try:
                        # Parse replay record
                        record = self.parse_replay_record(
                            row, i + 1, file_type, filename
                        )

                        # Process record
                        agrees, correction_field, correction_value = (
                            self.process_replay_record(record)
                        )

                        # Apply character replacement using core library utility
                        row[col_map["agrees"]] = agrees
                        row[col_map["correction_field"]] = (
                            self.char_replacer.colon_to_not_sign(correction_field)
                        )
                        row[col_map["correction_value"]] = (
                            self.char_replacer.colon_to_not_sign(correction_value)
                        )
                        ...
                        self.logger.error(
                            f"Error processing row {i + 1} in {filename}: {e}"
                        )
                        # Set error values
    ...
    def run(self):
        ...
        try:
            ...
            if not replay_files:
                self.logger.error(
                    "No CSV or xlsx replay files found in input directory"
                )
                return
            ...
            # Additional metrics
            total_indexed = sum(
                len(idx.transaction_ref_index) for idx in self.incident_indexes.values()
            )
