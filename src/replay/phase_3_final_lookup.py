...
from .phase_3_processor import IncidentColumnMapper as IncidentIdentityColumnMapper
from .phase_3_processor import IncidentFileIndex as IncidentIdentityIndex
...
class ClientKey:
    ...
    def __hash__(self):
        return hash(
            (self.id.lower(), self.first_name.lower(), self.surname.lower(), self.dob)
        )
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.first_name.lower() == other.first_name.lower()
...
class FieldMapper:
    ...
    @classmethod
    def get_unavista_index(
        cls, correction_field: str, client_type: str
    ) -> Optional[int]:
        """
...
class UnaVistaIndex:
    ...
    def __init__(self, file_paths: List[str], logger: logging.Logger):
        ...
        self.buyer_name_index = defaultdict(
            list
        )  # (first, last, dob) -> transaction_indices
        self.seller_name_index = defaultdict(
            list
        )  # (first, last, dob) -> transaction_indices
    ...
    def load_and_index(self):
        ...
        for file_path in self.file_paths:
            try:
                ...
                if len(rows) < 2:
                    self.logger.warning(
                        f"UnaVista file is empty or has no data rows: {os.path.basename(file_path)}"
                    )
                    continue
                ...
                data_rows = rows[1:]
                self.logger.info(
                    f"Loading {len(data_rows)} UnaVista transactions from {os.path.basename(file_path)}..."
                )
        ...
        self.logger.info(
            f"Indexed {len(self.transactions)} UnaVista transactions total"
        )
    ...
    def _index_buyer(self, row: List[str], idx: int):
        ...
        first_parts = (
            [p.strip().lower() for p in buyer_first_raw.split("|")]
            if buyer_first_raw
            else []
        )
        last_parts = (
            [p.strip().lower() for p in buyer_last_raw.split("|")]
            if buyer_last_raw
            else []
        )
        dob_parts = (
            [DateParser.parse_date(p.strip()) or "" for p in buyer_dob_raw.split("|")]
            if buyer_dob_raw
            else []
        )
        for buyer_first, buyer_last, buyer_dob in zip(
            first_parts, last_parts, dob_parts
        ):
    ...
    def _index_seller(self, row: List[str], idx: int):
        ...
        first_parts = (
            [p.strip().lower() for p in seller_first_raw.split("|")]
            if seller_first_raw
            else []
        )
        last_parts = (
            [p.strip().lower() for p in seller_last_raw.split("|")]
            if seller_last_raw
            else []
        )
        dob_parts = (
            [DateParser.parse_date(p.strip()) or "" for p in seller_dob_raw.split("|")]
            if seller_dob_raw
            else []
        )
    ...
    def _index_decision_makers(self, row: List[str], idx: int):
        ...
        dm_first_parts = (
            [p.strip().lower() for p in buyer_dm_first_raw.split("|")]
            if buyer_dm_first_raw
            else []
        )
        dm_last_parts = (
            [p.strip().lower() for p in buyer_dm_last_raw.split("|")]
            if buyer_dm_last_raw
            else []
        )
        dm_dob_parts = (
            [
        ...
        for buyer_dm_first, buyer_dm_last, buyer_dm_dob in zip(
            dm_first_parts, dm_last_parts, dm_dob_parts
        ):
        ...
        dm_first_parts = (
            [p.strip().lower() for p in seller_dm_first_raw.split("|")]
            if seller_dm_first_raw
            else []
        )
        dm_last_parts = (
            [p.strip().lower() for p in seller_dm_last_raw.split("|")]
            if seller_dm_last_raw
            else []
        )
        dm_dob_parts = (
            [
    ...
    def lookup_by_id(
        self, client_id: str, client_type: str
    ) -> List[UnaVistaTransaction]:
        """
    ...
    def lookup_by_name(
        self, first_name: str, surname: str, dob: str, client_type: str
    ) -> List[UnaVistaTransaction]:
        """
...
class ReplayRecordIndex:
    ...
    def __init__(
        self, logger: logging.Logger, incident_columns: Optional[Dict[str, str]] = None
    ):
        self.logger = logger
        ...
        self.logger.info(
            f"Loaded {len(self.incident_matrix)} incident code mappings from core library"
        )
    ...
    def load_replay_file(self, file_path: str, file_type: str):
        ...
        try:
            ...
            data_rows = rows[1:]
            self.logger.info(
                f"Processing {len(data_rows)} records from {file_type} file..."
            )
            ...
            self.logger.info(
                f"Indexed {len([r for r in self.records if r.source_file == file_type])} records from {file_type}"
            )
    ...
    def _parse_ids_record(
        ...
        try:
            ...
            incident_codes_str = (
                row[incident_codes_idx].strip() if len(row) > incident_codes_idx else ""
            )
            incident_codes = [
            ...
            correction_str = (
                row[correction_idx].strip() if len(row) > correction_idx else ""
            )
            field_str = (
                row[correction_field_idx].strip()
                if len(row) > correction_field_idx
                else ""
            )
            agree_str = (
                row[agree_idx].strip()
                if agree_idx is not None and len(row) > agree_idx
                else ""
            )
            suggested_str = (
                row[suggested_idx].strip()
                if suggested_idx is not None and len(row) > suggested_idx
                else ""
            )
            suggested_field_str = (
                row[suggested_field_idx].strip()
                if suggested_field_idx is not None and len(row) > suggested_field_idx
                else ""
            )
...
class Phase3FinalLookup:
    ...
    def __init__(
        ...
        self.data_reference_path = self.config.get("paths", {}).get(
            "unavista_files", ""
        )
        self.output_path = self.config.get("paths", {}).get("replay_output", "")
        self.log_output_path = self.config.get("paths", {}).get(
        ...
        self.incident_files_path: str = self.config.get("paths", {}).get(
            "incident_files", ""
        )

        # File patterns from config
        files_config = self.config.get("files", {})
        self.unavista_pattern = files_config.get(
            "unavista_pattern", "UnaVista_MiFIR_Manual_Corrections_*.csv"
        )
        self.replay_ids_pattern = files_config.get(
            "replay_ids_pattern", "Replay_*_Inconsistent_IDs_Summary_FINAL*.csv"
        )
        self.replay_names_pattern = files_config.get(
            "replay_names_pattern", "Replay_*_Inconsistent_Names_Summary_FINAL*.csv"
        )
        ...
        self.source_incident_columns: Dict[str, str] = self.config.get(
    ...
    def find_files(self):
        ...
        def find_file(search_path, pattern, description):
            ...
            if matches:
                ...
                return file_path
            self.logger.error(
                f"Could not find {description} matching pattern: {pattern}"
            )
            return None
        ...
        def find_all_files(search_path, pattern, description):
            matches = sorted(
                glob.glob(os.path.join(search_path, pattern)), key=os.path.getmtime
            )
            if matches:
                ...
                self.logger.error(
                    f"Could not find any {description} files matching pattern: {pattern}"
                )
            return matches

        # UnaVista: load ALL matching files
        self.unavista_paths = find_all_files(
            self.data_reference_path, self.unavista_pattern, "UnaVista"
        )
        self.replay_ids_path = find_file(
            self.replay_input_path, self.replay_ids_pattern, "Replay IDs file"
        )
        self.replay_names_path = find_file(
            self.replay_input_path, self.replay_names_pattern, "Replay Names file"
        )
        ...
        if not all([self.unavista_paths, self.replay_ids_path, self.replay_names_path]):
            raise FileNotFoundError(
                "Required input files not found. Please check file paths and patterns."
            )
    ...
    def load_indexes(self):
        ...
        if not all([self.replay_ids_path, self.replay_names_path, self.unavista_paths]):
            raise ValueError(
                "File paths not properly initialized. Call find_files() first."
            )
    ...
    def _extract_incident_correction(
        ...
        if correction_value:
            if agree in ("N", "F"):
                if suggested_value:
                    correction_value, correction_field = (
                        suggested_value,
                        suggested_field,
                    )
                else:
    ...
    def _parse_correction_to_dict(
        self, inc_value: str, inc_field: str
    ) -> Dict[str, str]:
    ...
    def _cross_reference_records(
        ...
        for code in all_incident_codes:
            ...
            if (
                ...
                and representative.first_name
                and representative.surname
            ):
            ...
            inc_value, inc_field = self._extract_incident_correction(
                row, index.column_mapper
            )
    ...
    def test_field(
        ...
        actual_value = (
            transaction.row_data[col_idx].strip()
            if col_idx < len(transaction.row_data)
            else ""
        )
    ...
    def find_matching_transactions(
        self, record: ReplayRecord, client_types: Set[str]
    ) -> Dict[str, List[UnaVistaTransaction]]:
        """
        ...
        for client_type in client_types:
            ...
            if record.client_id:
                for txn in self.unavista_index.lookup_by_id(
                    record.client_id, client_type
                ):
    ...
    def format_test_results(
        self, results_by_type: Dict[str, List[TestResult]], inconsistencies: List[str]
    ) -> str:
        """
        ...
        for client_type, results in results_by_type.items():
            ...
            if passed_results:
                pass_str = ", ".join(
                    [f"{r.field_name}={r.expected}" for r in passed_results]
                )
    ...
    def process_client(
        self, client_key: ClientKey, records: List[ReplayRecord]
    ) -> Dict[int, str]:
        """
        ...
        if inconsistencies:
            ...
            if client_types:
                matches_by_type = self.find_matching_transactions(
                    records[0], client_types
                )
                transaction_results = {}
                inconsistency_msg = "Inconsistent corrections: " + " | ".join(
                    inconsistencies
                )
        ...
        all_no_change = all(
            v.lower() == "no change" for v in merged_corrections.values()
        )
