    ...
    def _map_columns(self):
        ...
        for logical_name, column_name in self.column_config.items():
            try:
                ...
                if self.logger:
                    self.logger.warning(
                        f"Column '{column_name}' not found in incident file"
                    )
...
class IncidentFileIndex:
    ...
    def load_and_index(self):
        ...
        try:
            ...
            self.column_mapper = IncidentColumnMapper(
                self.header, self.column_config, self.logger
            )
            ...
            if self.logger:
                self.logger.info(
                    f"Loaded {os.path.basename(self.file_path)}: {len(self.data_rows)} rows"
                )
                self.logger.info(
                    f"  - Buyer IDs: {len(self.buyer_id_index)}, Seller IDs: {len(self.seller_id_index)}"
                )
                self.logger.info(
                    f"  - Buyer DM IDs: {len(self.buyer_dm_id_index)}, Seller DM IDs: {len(self.seller_dm_id_index)}"
                )
                self.logger.info(
                    f"  - Buyer Names: {len(self.buyer_name_index)}, Seller Names: {len(self.seller_name_index)}"
                )
                self.logger.info(
                    f"  - Buyer DM Names: {len(self.buyer_dm_name_index)}, Seller DM Names: {len(self.seller_dm_name_index)}"
                )
    ...
    def _build_indexes(self):
        ...
        for i, row in enumerate(self.data_rows):
            ...
            if (
                buyer_first_col is not None
                and buyer_last_col is not None
                and len(row) > max(buyer_first_col, buyer_last_col)
            ):
                buyer_first_parts = [
                    p.strip().lower() for p in row[buyer_first_col].split("|")
                ]
                buyer_last_parts = [
                    p.strip().lower() for p in row[buyer_last_col].split("|")
                ]
                buyer_dob_raw = (
                    row[buyer_dob_col]
                ...
                buyer_dob_parts = (
                    [
                        DateParser.parse_date(p.strip()) or ""
                        for p in buyer_dob_raw.split("|")
                    ]
                    if buyer_dob_raw
                    else [""]
                )

                for buyer_first, buyer_last, buyer_dob in zip(
                    buyer_first_parts, buyer_last_parts, buyer_dob_parts
                ):
            ...
            if (
                seller_first_col is not None
                and seller_last_col is not None
                and len(row) > max(seller_first_col, seller_last_col)
            ):
                seller_first_parts = [
                    p.strip().lower() for p in row[seller_first_col].split("|")
                ]
                seller_last_parts = [
                    p.strip().lower() for p in row[seller_last_col].split("|")
                ]
                seller_dob_raw = (
                    row[seller_dob_col]
                ...
                seller_dob_parts = (
                    [
                        DateParser.parse_date(p.strip()) or ""
                        for p in seller_dob_raw.split("|")
                    ]
                    if seller_dob_raw
                    else [""]
                )

                for seller_first, seller_last, seller_dob in zip(
                    seller_first_parts, seller_last_parts, seller_dob_parts
                ):
            ...
            if (
                buyer_dm_first_col is not None
                and buyer_dm_last_col is not None
                and len(row) > max(buyer_dm_first_col, buyer_dm_last_col)
            ):
                buyer_dm_first_parts = [
                    p.strip().lower() for p in row[buyer_dm_first_col].split("|")
                ]
                buyer_dm_last_parts = [
                    p.strip().lower() for p in row[buyer_dm_last_col].split("|")
                ]
                buyer_dm_dob_raw = (
                    row[buyer_dm_dob_col]
                ...
                buyer_dm_dob_parts = (
                    [
                        DateParser.parse_date(p.strip()) or ""
                        for p in buyer_dm_dob_raw.split("|")
                    ]
                    if buyer_dm_dob_raw
                    else [""]
                )

                for buyer_dm_first, buyer_dm_last, buyer_dm_dob in zip(
                    buyer_dm_first_parts, buyer_dm_last_parts, buyer_dm_dob_parts
                ):
            ...
            if (
                seller_dm_first_col is not None
                and seller_dm_last_col is not None
                and len(row) > max(seller_dm_first_col, seller_dm_last_col)
            ):
                seller_dm_first_parts = [
                    p.strip().lower() for p in row[seller_dm_first_col].split("|")
                ]
                seller_dm_last_parts = [
                    p.strip().lower() for p in row[seller_dm_last_col].split("|")
                ]
                seller_dm_dob_raw = (
                    row[seller_dm_dob_col]
                ...
                seller_dm_dob_parts = (
                    [
                        DateParser.parse_date(p.strip()) or ""
                        for p in seller_dm_dob_raw.split("|")
                    ]
                    if seller_dm_dob_raw
                    else [""]
                )

                for seller_dm_first, seller_dm_last, seller_dm_dob in zip(
                    seller_dm_first_parts, seller_dm_last_parts, seller_dm_dob_parts
                ):
    ...
    def _find_best_row_idx(self, row_indices: List[int]) -> int:
        ...
        """
        correction_col = (
            self.column_mapper.get("correction") if self.column_mapper else None
        )
        suggested_col = (
            self.column_mapper.get("suggested_correction")
    ...
    def lookup_by_id(
        self, client_ids: List[str], client_first: str = "", client_surname: str = ""
        ...
        for client_id in client_ids:
            ...
            if client_id_lower in self.buyer_id_index:
                    ...
                    if buyer_first_col is not None and buyer_last_col is not None:
                        ...
                        for row_idx in row_indices:
                            ...
                            if len(row) > max(buyer_first_col, buyer_last_col):
                                ...
                                if (
                                    first == client_first.lower()
                                    and last == client_surname.lower()
                                ):
                        ...
                        if name_matched:
                            ...
                            if self.logger:
                                self.logger.debug(
                                    f"Found ID '{client_id}' with name match in buyer_id_index (row {row_idx})"
                                )
                ...
                if self.logger:
                    self.logger.debug(
                        f"Found ID '{client_id}' in buyer_id_index (row {row_idx}, {len(row_indices)} total matches)"
                    )
                return (row_idx, "id_buyer")
            ...
            if client_id_lower in self.seller_id_index:
                ...
                if len(row_indices) > 1 and client_first and client_surname:
                    ...
                    if seller_first_col is not None and seller_last_col is not None:
                        ...
                        if name_matched:
                            ...
                            if self.logger:
                                self.logger.debug(
                                    f"Found ID '{client_id}' with name match in seller_id_index (row {row_idx})"
                                )
                ...
                if self.logger:
                    self.logger.debug(
                        f"Found ID '{client_id}' in seller_id_index (row {row_idx}, {len(row_indices)} total matches)"
                    )
                return (row_idx, "id_seller")
            ...
            if client_id_lower in self.buyer_dm_id_index:
                ...
                if len(row_indices) > 1 and client_first and client_surname:
                    ...
                    if buyer_dm_first_col is not None and buyer_dm_last_col is not None:
                        ...
                        if name_matched:
                            ...
                            if self.logger:
                                self.logger.debug(
                                    f"Found ID '{client_id}' with name match in buyer_dm_id_index (row {row_idx})"
                                )
                ...
                if self.logger:
                    self.logger.debug(
                        f"Found ID '{client_id}' in buyer_dm_id_index (row {row_idx}, {len(row_indices)} total matches)"
                    )
                return (row_idx, "id_buyer_dm")
            ...
            if client_id_lower in self.seller_dm_id_index:
                ...
                if len(row_indices) > 1 and client_first and client_surname:
                    ...
                    if (
                        ...
                        if name_matched:
                            ...
                            if self.logger:
                                self.logger.debug(
                                    f"Found ID '{client_id}' with name match in seller_dm_id_index (row {row_idx})"
                                )
                ...
                if self.logger:
                    self.logger.debug(
                        f"Found ID '{client_id}' in seller_dm_id_index (row {row_idx}, {len(row_indices)} total matches)"
                    )
                return (row_idx, "id_seller_dm")
    ...
    def lookup_by_name(
        self, first_name: str, surname: str, dob: str
    ) -> Optional[Tuple[int, str]]:
        ...
        if dob_parsed:
            ...
            if name_key_no_dob in self.buyer_name_index:
                row_idx = self._find_best_row_idx(
                    self.buyer_name_index[name_key_no_dob]
                )
                return (row_idx, "name_buyer")

            if name_key_no_dob in self.seller_name_index:
                row_idx = self._find_best_row_idx(
                    self.seller_name_index[name_key_no_dob]
                )
                return (row_idx, "name_seller")
        ...
        if dob_parsed:
            ...
            if name_key_no_dob in self.buyer_dm_name_index:
                row_idx = self._find_best_row_idx(
                    self.buyer_dm_name_index[name_key_no_dob]
                )
                return (row_idx, "name_buyer_dm")

            if name_key_no_dob in self.seller_dm_name_index:
                row_idx = self._find_best_row_idx(
                    self.seller_dm_name_index[name_key_no_dob]
                )
                return (row_idx, "name_seller_dm")
...
class Phase3Processor:
    ...
    def __init__(
        ...
        if not replay_patterns:
            raise ValueError(
                "Configuration error: 'files.replay_patterns' is required in config file"
            )
        ...
        if not self.replay_files:
            raise ValueError(
                f"No replay files found matching patterns: {replay_patterns}"
            )
        ...
        if self.replace_from and not self.replace_to:
            raise ValueError(
                "Configuration error: 'replace_pattern.to' is required when 'replace_pattern.from' is specified"
            )
        if self.replace_to and not self.replace_from:
            raise ValueError(
                "Configuration error: 'replace_pattern.from' is required when 'replace_pattern.to' is specified"
            )

        # Similarity threshold for fuzzy matching (reasonable technical default)
        self.similarity_threshold = self.config.get("processor", {}).get(
        ...
        if not incident_pattern:
            raise ValueError(
                "Configuration error: 'files.incident_pattern' is required in config file"
            )
        ...
        if not self.incident_columns:
            raise ValueError(
                "Configuration error: 'incident_columns' section is required in config file"
            )

    def __del__(self):
        """Best-effort logger cleanup for short-lived processor instances."""
        try:
            self.logger.close()
        except Exception:
            pass
    ...
    def preload_and_index_incident_files(self):
        ...
        for replay_filename in self.replay_files:
            replay_filepath = os.path.join(
                self.path_config.replay_input, replay_filename
            )
        ...
        self.logger.info(
            f"Loading and indexing {len(incident_codes)} incident files..."
        )
        ...
        for incident_code in incident_codes:
            ...
            if incident_file:
                ...
                self.incident_indexes[incident_code] = IncidentFileIndex(
                    incident_file, self.incident_columns, self.logger
                )
                loaded_count += 1
    ...
    def find_incident_file(self, incident_code: str) -> Optional[str]:
        ...
        filepath_with_dash = os.path.join(
            self.path_config.incident_files, pattern_with_dash
        )
        ...
        glob_pattern = os.path.join(
            self.path_config.incident_files, f"* {incident_code}.csv"
        )
    ...
    def parse_client_record_ids(self, row: List[str], row_index: int) -> ClientRecord:
        ...
        try:
            ...
            incident_codes = [
                code.strip() for code in incident_codes_str.split("|") if code.strip()
            ]
    ...
    def lookup_client(self, client: ClientRecord, incident_code: str) -> LookupResult:
        ...
        if client.all_ids and any(client.all_ids):
            id_result = index.lookup_by_id(
                client.all_ids, client.first_name, client.surname
            )
            if id_result:
                row_idx, match_type = id_result
                return self._create_lookup_result(
                    index.data_rows[row_idx], index.column_mapper, match_type
                )

        # Try name lookup (O(1) with index)
        if client.first_name and client.surname:
            name_result = index.lookup_by_name(
                client.first_name, client.surname, client.date_of_birth
            )
            if name_result:
    ...
    def _create_lookup_result(
        self, row: List[str], column_mapper: IncidentColumnMapper, match_type: str
    ) -> LookupResult:
        ...
        try:
            ...
            suggested_correction_field_col = column_mapper.get(
                "suggested_correction_field"
            )

            # Extract transaction reference
            transaction_ref = (
                row[txn_ref_col].strip()
                if txn_ref_col is not None and len(row) > txn_ref_col
                else ""
            )

            # Extract correction values
            correction_value = (
                row[correction_col].strip()
                if correction_col is not None and len(row) > correction_col
                else ""
            )
            correction_field_value = (
                row[correction_field_col].strip()
                if correction_field_col is not None and len(row) > correction_field_col
                else ""
            )

            # Extract suggested correction values
            suggested_correction_value = (
                row[suggested_correction_col].strip()
                if suggested_correction_col is not None
                and len(row) > suggested_correction_col
                else ""
            )
            suggested_correction_field_value = (
                row[suggested_correction_field_col].strip()
                if suggested_correction_field_col is not None
                and len(row) > suggested_correction_field_col
                else ""
            )
            ...
            if correction_value:
                ...
                if agree_value in ("Y", "P", ""):
                    ...
                    if self.logger:
                        self.logger.debug(
                            f"Applying Correction (Agree={agree_value or 'empty'}, match={match_type})"
                        )
                # If agree is N or F -> check Suggested Correction
                    ...
                    if suggested_correction_value:
                        ...
                        if self.logger:
                            self.logger.debug(
                                f"Applying Suggested Correction (analyst disagreed, match={match_type})"
                            )
                    else:
                        if self.logger:
                            self.logger.debug(
                                f"No correction (analyst disagreed, no suggestion, match={match_type})"
                            )
                else:
                    ...
                    if self.logger:
                        self.logger.warning(
                            f"Unknown Agree value '{agree_value}', defaulting to Correction"
                        )
            else:
                ...
                if suggested_correction_value:
                    ...
                    if self.logger:
                        self.logger.debug(
                            f"Applying Suggested Correction (no automated correction, match={match_type})"
                        )
                else:
                    if self.logger:
                        self.logger.debug(
                            f"No correction to apply (match={match_type})"
                        )
            ...
            if re.search(r"\bre\s+accounts?\b", correction, re.IGNORECASE):
                if self.logger:
                    self.logger.debug(
                        f"RE Account correction detected, treating as No Change (match={match_type})"
                    )
                correction = "No Change"
                correction_field = ""
    ...
    def process_client_record(self, client: ClientRecord) -> Tuple[str, str]:
        ...
        if not client.incident_codes:
            self.logger.debug(
                f"No incident codes for client: {client.id_value} {client.first_name} {client.surname}"
            )
        ...
        unique_corrections = list(
            set(zip(all_correction_values, all_correction_fields))
        )
            ...
            for batch_start in range(0, total_rows, batch_size):
                ...
                for i in range(batch_start, batch_end):
                    ...
                    try:
                        ...
                        # Process record
                        correction, correction_field = self.process_client_record(
                            client
                        )
    ...
    def run(self):
        ...
        try:
            ...
            # Additional metrics
            total_indexed = sum(
                len(idx.buyer_id_index) + len(idx.seller_id_index)
                for idx in self.incident_indexes.values()
            )
