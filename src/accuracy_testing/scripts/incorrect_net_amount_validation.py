...
from core import StructuredLogger, create_logger, safe_open_csv
...
class IncorrectNetAmountValidationScript:
    ...
    def __init__(
        ...
        self.validator = IncorrectNetAmountValidator(
            tolerance=Decimal("0.01"), verbose=self.proc_config.verbose
        )
        ...
        try:
            with f:
                ...
                if self.has_header:
                    ...
                    self.logger.info(
                        "Header mode: HEADERLESS — using fixed SQL extract column positions"
                    )
                ...
                for row_idx, row in enumerate(reader, start=first_data_row):
                    ...
                    try:
                        sedol_val = (
                            row[sedol_col].strip()
                            if sedol_col is not None and sedol_col < len(row)
                            else None
                        )
                        instrument_val = (
                            row[instrument_col].strip()
                            if instrument_col is not None and instrument_col < len(row)
                            else None
                        )
                        ...
                        record = IncorrectNetAmountRecord(
                            transaction_ref=(
                                row[self.COL_TRANSACTION_REF].strip()
                                if row[self.COL_TRANSACTION_REF]
                                else ""
                            ),
                            net_amount=_to_decimal(
                                row[self.COL_NET_AMOUNT], "Net Amount", row_idx
                            ),
                            consideration=_to_decimal(
                                row[self.COL_CONSIDERATION], "Consideration", row_idx
                            ),
                            interest=_to_decimal(
                                row[self.COL_INTEREST], "Interest", row_idx
                            ),
    ...
    def run(self):
        ...
        if self.dry_run:
            ...
            self.logger.info(
                f"Would have written {len(records)} records to: {self.output_file}"
            )
...
def run_batch_validation(
    ...
    if incidents_config == "auto":
        ...
        print(
            f"Auto-discovered {len(incidents)} pricing incident(s): {', '.join(incidents)}"
        )
