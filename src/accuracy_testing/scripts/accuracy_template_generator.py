...
def print_summary(
    generator: AccuracyTemplateGenerator, output_dir: str, dry_run: bool = False
):
...
def main():
    ...
    if args.config:
        ...
        default_config = (
            ...
            / "local"
            / "accuracy_testing"
            / "accuracy_template_generator.yaml"
        )
    ...
    if not errors_path and not queries_path:
        print(
            "ERROR: At least one input file (--errors, --queries, or via --config) is required"
        )
        sys.exit(1)
    ...
    print(
        f"Mode:          {'DRY RUN (preview only)' if args.dry_run else 'Generate files'}"
    )
    print("=" * 70)
    ...
    try:
        ...
        if args.dry_run:
            ...
            for incident_code in sorted(generator.incident_records.keys()):
                ...
                filename = (
                    f"{fy_q_prefix}{incident_code}.csv"
                    if (fiscal_year and quarter)
                    else f"template_{incident_code}.csv"
                )
            ...
            # Generate templates
            generated_files = generator.generate_templates(
                str(output_dir), fiscal_year, quarter
            )
