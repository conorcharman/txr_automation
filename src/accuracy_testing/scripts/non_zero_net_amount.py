...
5. Compares the net sum against parent_netamt
6. Outputs each record with error flag, net_amt, and difference columns
...
Output CSV columns (all input columns + 3 appended):
    ...
    - report_status
    - trade_date_time
    - net_amt        (sum of child_netamt for this parent group)
...
class NonZeroNetAmountScript:
    ...
    # Output CSV column headers — input columns, then bulk_ref/bulk_netamt after
    # parent_netamt, then the three calculated columns.
    OUTPUT_COLUMNS = [
        "child_ref",
        "child_netamt",
        "parent_ref",
    ...
    def write_output_csv(self, records: List[NetAmountRecord]) -> None:
        ...
        here. The output contains all input columns plus net_amt, difference,
        and error.
    ...
    def run(self) -> None:
        ...
        if self.dry_run:
            ...
            if output_records:
                ...
                self.logger.info(f"  child_netamt:  {sample.child_netamt}")
                self.logger.info(f"  net_amt:       {sample.net_amt}")
                self.logger.info(f"  parent_netamt: {sample.parent_netamt}")
