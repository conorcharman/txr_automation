...
class DataPushCLI:
    ...
    def __init__(
        ...
        # Get file paths
        self.source_file = Path(paths.get("source_file", "")) if paths.get("source_file") else None
        self.target_file = Path(paths.get("target_file", "")) if paths.get("target_file") else None
        self.output_file = Path(paths.get("output_file", "")) if paths.get("output_file") else None
    ...
    def _print_summary(self, processor: BatchDataPushProcessor) -> None:
        ...
        if self.verbose:
            ...
            for incident, stats in summary["by_incident"].items():
                print(f"  {incident}: {stats['matched']}/{stats['total_source']} matched")
