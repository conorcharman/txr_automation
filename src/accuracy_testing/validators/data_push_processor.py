...
class DataPushProcessor:
    ...
    def match_records(self) -> Tuple[int, int]:
        ...
        for record in self.source_records:
            if record.transaction_ref in self.target_index:
                ...
                not_found += 1
                self.logger.debug(f"Not found: {record.transaction_ref}")
    ...
    def push_data(self) -> PushStats:
        ...
        for record in self.source_records:
            ...
            try:
                ...
                self.stats.errors += 1
                self.logger.error(f"Error pushing {record.transaction_ref}: {e}")
        ...
        if record.action == PushAction.UPDATE_ALL:
            ...
            if error_target_col and error_target_col in self.target_df.columns:
                ...
                if self.config.error_column in self.target_df.columns:
                    self.target_df.at[
                        record.target_row_index, self.config.error_column
                    ] = "N"
                    record.push_result = "Updated error flag to N"
    ...
    def get_unmatched_records(self) -> List[DataPushRecord]:
        ...
        """
        return [r for r in self.source_records if r.action == PushAction.NOT_FOUND]
...
class BatchDataPushProcessor:
    ...
    def __init__(
        ...
        self.backup_dir = (
            Path(backup_dir) if backup_dir else Path(base_target_dir) / "backup"
        )
        self.logger = logger or logging.getLogger(__name__)
        ...
        for pattern in patterns:
            for file in self.base_source_dir.glob(pattern):
                ...
                if name.startswith(f"validated_{self.fiscal_year}_{self.quarter}_"):
                    ...
                    suffix = name.replace(
                        f"validated_{self.fiscal_year}_{self.quarter}_", ""
                    )
    ...
    def get_batch_summary(self) -> Dict[str, Any]:
        ...
        total_updated = sum(
            s.updated_all + s.updated_error_only for s in self.results.values()
        )
        ...
        return {
            ...
            "by_incident": {
                code: stats.as_dict() for code, stats in self.results.items()
            },
        }
