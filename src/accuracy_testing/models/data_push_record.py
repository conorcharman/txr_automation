...
class DataPushRecord:
    ...
    def get_push_values(self, column_mappings: List[ColumnMapping]) -> Dict[str, Any]:
        """
        ...
        if self.action == PushAction.UPDATE_ALL:
            ...
            for mapping in column_mappings:
                if mapping.source_col in self.source_data:
                    ...
                    if skip_corrections and mapping.source_col in (
                        "Correction",
                        "Correction Field",
                    ):
                        continue
...
DEFAULT_COLUMN_MAPPINGS = [
    ...
    ColumnMapping(
        "Type of Seller ID Code", "Type of Seller ID Code", "Type of seller ID"
    ),
    # Personal details
    ...
    ColumnMapping(
        ...
        "Prefixed Nationality",
        "ISO-2 prefixed nationality code",
    ),
    ColumnMapping(
        "Primary Nationality", "Primary Nationality", "Primary nationality code"
    ),
    ColumnMapping(
        "Secondary Nationality", "Secondary Nationality", "Secondary nationality code"
    ),
    ...
    ColumnMapping(
        "Failure Reason", "Failure Reason", "Specific reason for validation failure"
    ),
    ColumnMapping("Correction", "Correction", "Suggested correction (ID:TYPE format)"),
    ColumnMapping(
        "Correction Field", "Correction Field", "Fields being corrected (ID:IDT format)"
    ),
    ColumnMapping(
        "Agree With Correction",
        "Agree With Correction",
        "Analyst agreement with correction",
    ),
    ColumnMapping(
        "Suggested Correction",
        "Suggested Correction",
        "Analyst suggested alternative correction",
    ),
    ColumnMapping(
        "Suggested Correction Field",
        "Suggested Correction Field",
        "Fields for analyst correction",
    ),
    # Status columns
    ...
    ColumnMapping(
        "Net Difference", "Net Difference", "Difference between net amount and expected"
    ),
    ...
    ColumnMapping(
        ...
        "Child transaction reference — match key for net qty/amt incidents",
    ),
    ColumnMapping("parent_ref", "parent_ref", "Parent order reference"),
    ColumnMapping(
        "bulk_ref", "bulk_ref", "Contract group prefix derived from parent_ref"
    ),
    ...
    ColumnMapping(
        "error", "error", "Validation error flag, lowercase (N=match, Y=mismatch)"
    ),
    # Incorrect Time (7_30) columns
    ColumnMapping(
        "child_datetime", "child_datetime", "Child transaction datetime (7_30)"
    ),
    ColumnMapping("parent_datetime", "parent_datetime", "Parent order datetime (7_30)"),
    ColumnMapping(
        "time_difference",
        "time_difference",
        "Time difference between child and parent (7_30)",
    ),
]
