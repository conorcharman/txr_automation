...
class SQLExtractGenerator:
    ...
    def __init__(
        ...
        if self.output_format in ["dtf", "both"]:
            if dtf_template_path:
                ...
                self.dtf_template_path = (
                    self.template_path.parent / "AS400_DataTransfer_template.dtf"
                )

            if not self.dtf_template_path.exists():
                raise FileNotFoundError(
                    f"DTF template file not found: {self.dtf_template_path}"
                )
    ...
    def create_batches(self, transaction_refs: List[str]) -> List[ExtractBatch]:
        ...
        for i in range(0, len(transaction_refs), self.batch_size):
            ...
            batches.append(
                ExtractBatch(batch_number=batch_number, transaction_refs=batch_refs)
            )
            batch_number += 1
    ...
    def format_values_block(self, refs: List[str]) -> str:
        ...
        for ref in refs:
            ...
            if clean_ref.upper().startswith("CA"):
                continue
            k_firm, k_year, k_accl, k_cont, k_suff = self.split_transaction_ref(
                clean_ref
            )
            rows.append(
        ...
        for batch in batches:
            ...
            if self.output_format in ["sql", "both"]:
                sql_path = self.write_sql_file(
                    sql_dir, base_filename, batch, sql, total_batches=total_batches
                )
