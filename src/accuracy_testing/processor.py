...
class AccuracyConfigManager:
    ...
    def load_from_env(prefix: str = "TXR_ACCURACY_") -> Dict[str, Any]:
        ...
        config["processor"]["log_level"] = os.getenv(
            f"{prefix}PROCESSOR_LOG_LEVEL", "INFO"
        )
        config["processor"]["verbose"] = (
            os.getenv(f"{prefix}PROCESSOR_VERBOSE", "false").lower() == "true"
        )
        config["processor"]["batch_size"] = int(
            os.getenv(f"{prefix}PROCESSOR_BATCH_SIZE", "1000")
        )
    ...
    def get_path_config(config: Dict[str, Any]) -> AccuracyPathConfig:
        ...
        if missing_keys:
            raise ValueError(
                f"Missing required path configuration: {', '.join(missing_keys)}"
            )
...
class ClientRecord:
    ...
    requires_standard_validation: bool = (
        True  # False if corrected by inconsistent ID logic
    )
...
class ProcessingStats:
    ...
    def print_summary(self, logger=None):
        ...
        if logger and LOGGER_AVAILABLE:
            ...
            logger.info(
                f"No correction possible:      {self.no_correction_possible:>6}"
            )
            ...
            if self.errors_by_country:
                ...
                for country, count in sorted(
                    self.errors_by_country.items(), key=lambda x: x[1], reverse=True
                ):
...
class InconsistentIDStats:
    ...
    corrected_to_most_recent: int = (
        0  # Invalid/fallback IDs corrected to most recent valid
    )
    valid_standardized: int = (
        0  # Valid but different IDs corrected to most recent valid
    )
...
class InconsistentIDProcessor:
    ...
    @staticmethod
    def is_fallback_id_pattern(
        id_value: str, person_code: str, country_code: str
    ) -> bool:
        """
    ...
    def has_inconsistent_ids(
        self, records: List[ClientRecord], indices: List[int]
    ) -> bool:
        """
        ...
        for prefix, group in prefix_groups.items():
            ...
            if first_prefix:
                first_id_stripped = (
                    first_id[len(first_prefix) :]
                    if first_id.startswith(first_prefix)
                    else first_id
                )
            else:
    ...
    def _extract_nationality_prefix(self, record: ClientRecord) -> Optional[str]:
        ...
        if len(id_value) >= 2:
            ...
            if potential_prefix.isalpha() and country_manager.validate_code(
                potential_prefix
            ):
    ...
    def validate_id_complete(
        self, record: ClientRecord, id_processor: "IDValidationProcessor"
        ...
        if format_valid:
            ...
            if not logic_valid:
                logic_error = (
                    id_processor.id_logic_validator.last_failure_reason
                    or "Logic validation failed"
                )
    ...
    def _find_most_recent_valid_id(
        self, records: List[ClientRecord], indices: List[int], id_type: str, prefix: str
        ...
        for idx in indices:
            ...
            if (
                ...
                and record.prefixed_nationality == prefix
                and record.is_valid_id
                and not record.is_fallback_id
            ):

                # Compare datetimes to find most recent
                if most_recent_datetime is None or (
                    record.trade_date_time_parsed
                    and record.trade_date_time_parsed > most_recent_datetime
                ):
                ...
                for idx in group_indices:
                    ...
                    if prefix:
                        record.correction_source = (
                            f"No valid {id_type} with prefix {prefix} - will use CONCAT"
                        )
                        record.actions_taken.append(
                            f"Inconsistent ID - No valid {id_type} with prefix {prefix}, using CONCAT"
                        )
                    else:
                        record.correction_source = (
                            f"No valid {id_type} found - requires standard correction"
                        )
                        record.actions_taken.append(
                            f"Inconsistent ID - No valid {id_type}, using standard correction"
                        )
                    self.stats.no_valid_in_group += 1
                    # Note: failure_reason is preserved from validation phase above

                self._log(
                    f"ID Type {id_type} (prefix: {prefix}): No valid ID found, marking {len(group_indices)} records for standard correction"
                )
                continue
            ...
            for idx in group_indices:
                ...
                if record.id_value == most_recent_id:
                    ...
                    if record.is_valid_id and not record.is_fallback_id:
                        record.actions_taken.append(
                            f"Pass - Most recent valid {id_type}"
                        )
                    ...
                    self._log(
                        f"Record {record.row_index}: Already matches most recent valid {id_type}: {most_recent_id}"
                    )
                else:
                    ...
                    if was_valid:
                        ...
                        record.actions_taken.append(
                            f"Inconsistent ID - Valid {id_type} standardized to most recent: {most_recent_id}"
                        )
                        self.stats.valid_standardized += 1
                        self._log(
                            f"Record {record.row_index}: Standardized valid {id_type} '{record.id_value}' to most recent: {most_recent_id}"
                        )
                    else:
                        # Invalid or fallback ID - correcting to most recent
                        record.actions_taken.append(
                            f"Inconsistent ID - Corrected to most recent valid {id_type} (prefix: {prefix})"
                        )
                        self.stats.corrected_to_most_recent += 1
                        self._log(
                            f"Record {record.row_index}: Corrected invalid {id_type} to most recent: {most_recent_id}"
                        )

    def preprocess_for_inconsistent_validation(
        self, records: List[ClientRecord], id_processor: "IDValidationProcessor"
    ) -> List[ClientRecord]:
        """
        ...
        for record in records:
            ...
            if not record.priority_country_code:
                record.priority_country_code = id_processor._get_priority_country(
                    record
                )
        ...
        for person_code, record_indices in person_groups.items():
            ...
            self._log(
                f"Person {person_code}: Found {len(record_indices)} records with inconsistent IDs"
            )
            ...
            for idx in record_indices:
                ...
                if record.id_value and not record.id_type:
                    ...
                    if id_prefix:
                        ...
                        is_concat = id_format_manager.validate(
                            id_prefix, "CONCAT", record.id_value
                        )
                        ...
                        if is_concat:
                            ...
                            record.actions_taken.append(
                                f"Auto-detected ID type as CONCAT (prefix: {id_prefix})"
                            )
                            self._log(
                ...
                record.is_fallback_id = self.is_fallback_id_pattern(
                    record.id_value, record.person_code, record.priority_country_code
                )
                ...
                if record.is_fallback_id:
                    ...
                    format_valid, logic_valid, error_message = (
                        self.validate_id_complete(record, id_processor)
                    )
                    ...
                    if format_valid and logic_valid:
                        ...
                        if (
                            record.priority_country_code == "SE"
                            and record.id_type == "NIDN"
                            and record.id_value
                        ):
                            # Check if ID is 10 digits (after stripping country prefix)
                            id_without_prefix = (
                                record.id_value[2:]
                                if len(record.id_value) > 2
                                and record.id_value[:2].upper() == "SE"
                                else record.id_value
                            )
                            if (
                                len(id_without_prefix) == 10
                                and id_without_prefix.isdigit()
                            ):
                                ...
                                if record.date_of_birth:
                                    try:
                                        ...
                                        dob_result = validate_date_format(
                                            record.date_of_birth, "%Y-%m-%d"
                                        )
                                        if not dob_result.is_valid:
                                            dob_result = validate_date_format(
                                                record.date_of_birth, "%d/%m/%Y"
                                            )
                                        if not dob_result.is_valid:
                                            dob_result = validate_date_format(
                                                record.date_of_birth, "%d-%m-%Y"
                                        ...
                                        if (
                                            dob_result.is_valid
                                            and dob_result.corrected_value
                                        ):
                                            century = str(
                                                dob_result.corrected_value.year
                                            )[
                                                :2
                                            ]  # "19" or "20"
                                            ...
                                            if normalized_id != record.id_value:
                                                ...
                                                self._log(
                                                    f"Normalized SE NIDN from 10 to 12 digits: {id_without_prefix} -> {normalized_id}"
                                                )
                                    except Exception as e:
                                        self._log(
                                            f"Warning: Could not normalize SE NIDN: {e}"
                                        )
                        ...
                        record.failure_reason = (
                            error_message or f"{record.id_type} logic validation failed"
                        )
                        ...
                        record.logic_status = "N/A"
                        record.failure_reason = (
                            error_message or f"Invalid {record.id_type} format"
                        )
                        self.stats.invalid_ids_found += 1
                    else:
        ...
        self._log(
            f"  - Invalid corrected to most recent: {self.stats.corrected_to_most_recent}"
        )
        self._log(
            f"  - Valid standardized to most recent: {self.stats.valid_standardized}"
        )
        self._log(
            f"  - No valid in group (need standard): {self.stats.no_valid_in_group}"
        )
        self._log(
            f"  - Already most recent (no change): {self.stats.already_most_recent}"
        )
...
class IDValidationProcessor:
    ...
    def process_record(self, record: ClientRecord) -> ClientRecord:
        ...
        if record.id_value and record.id_type:
            ...
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[VALIDATION] Row {record.row_index}: id_value='{record.id_value}', id_type='{record.id_type}', country='{country_code}', valid={is_valid}"
                    )
            ...
            if is_valid:
                ...
                if self.verbose:
                    if self.logger:
                        self.logger.debug(
                            f"[LOGIC CHECK] Row {record.row_index}: Calling logic validator with id='{record.id_value}', type='{record.id_type}', country='{country_code}', dob='{record.date_of_birth}', gender='{record.gender}'"
                        )
                ...
                if self.verbose:
                    if self.logger:
                        self.logger.debug(
                            f"[LOGIC CHECK] Row {record.row_index}: Logic validation result = {logic_valid}"
                        )
                ...
                if not logic_valid:
                    ...
                    self.stats.track_error(
                        country_code, record.id_type, record.failure_reason
                    )
                else:
                    record.is_valid = True
                    record.format_status = "Pass"
                            ...
                            if record.date_of_birth:
                                try:
                                    ...
                                    if (
                                        ...
                                        century = str(dob_result.corrected_value.year)[
                                            :2
                                        ]  # "19" or "20"
                                        # Reconstruct with century: SE + CC + YYMMDDBBBC
                                        ...
                                        if normalized_id != record.id_value:
                                            if self.verbose and self.logger:
                                                self.logger.debug(
                                                    f"Normalized SE NIDN from 10 to 12 digits: {record.id_value} -> {normalized_id}"
                                                )
                                            record.id_value = normalized_id
                                except Exception as e:
                                    if self.verbose and self.logger:
                                        self.logger.warning(
                                            f"Could not normalize SE NIDN: {e}"
                                        )
                ...
                record.validation_error = (
                    f"Invalid {record.id_type} format for {country_code}"
                )
                record.format_status = "Fail"
                record.logic_status = "N/A"
            ...
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[VALIDATION] Row {record.row_index}: Missing ID or type - id_value='{record.id_value}', id_type='{record.id_type}'"
                    )
            record.is_valid = False
            ...
            self.stats.invalid_format += 1
            self.stats.track_error(
                country_code, record.id_type or "Unknown", record.failure_reason
            )
        ...
        if correction:
            ...
            record.actions_taken = [
                ...
                if not action.startswith("INVALID:")
            ]
            ...
            elif record.correction_type == "NIDN" and correction[0].startswith(
                country_code
            ):
            ...
            if not record.person_code:
                ...
                debug_msg += (
                    f" All generation methods failed (person_code={record.person_code})"
                )
    ...
    def _generate_correction(
        self, record: ClientRecord, country_code: str
        ...
        if self.verbose:  # DEBUG level
            if self.logger:
                self.logger.debug(
                    f"[CORRECTION] Attempting correction for row {record.row_index}"
                )
        ...
        if record.id_value:
            ...
            if detected_type:
                if self.verbose:
                    if self.logger:
                        self.logger.debug(
                            f"[CORRECTION] Alternative type found: {detected_type}"
                        )
        ...
        if concat_supported:
            ...
            if concat_id:
                ...
                if is_eea:
                    is_valid = id_format_manager.validate(
                        country_code, "CONCAT", concat_id
                    )
                    if self.verbose:
                        if self.logger:
                            self.logger.debug(
                                f"[CORRECTION] CONCAT validation result (EEA): {is_valid}"
                            )
                    if is_valid:
                    ...
                    if self.verbose:
                        if self.logger:
                            self.logger.debug(
                                f"[CORRECTION] Using CONCAT for Rest of World country (no validation)"
                            )
                    return (concat_id, "CONCAT")
            else:
                if self.verbose:
                    if self.logger:
                        self.logger.debug(
                            f"[CORRECTION] CONCAT generation failed - missing data (fname={bool(record.first_name)}, sname={bool(record.surname)}, dob={bool(record.date_of_birth)})"
                        )
        else:
            ...
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[CORRECTION] Country {country_code} does not support CONCAT - skipping to fallback ID"
                    )
        ...
        if record.person_code:
            ...
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[CORRECTION] Generated fallback ID: {fallback_id}"
                    )
        ...
        if self.verbose:
            if self.logger:
                self.logger.debug(
                    f"[CORRECTION] No correction possible - missing person_code"
                )
        return None

    def _test_alternative_types(
        self, id_value: str, country_code: str
    ) -> Optional[str]:
        """
    ...
    def _try_swedish_century_fix(
        ...
        id_without_prefix = (
            record.id_value[2:] if len(record.id_value) > 2 else record.id_value
        )
    ...
    def _generate_concat(
        ...
        if not all([record.first_name, record.surname, record.date_of_birth]):
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[CONCAT] Missing required fields - fname={bool(record.first_name)}, sname={bool(record.surname)}, dob={bool(record.date_of_birth)}"
                    )
            return None
        ...
        if not dob_result.is_valid:
            if self.verbose:
                if self.logger:
                    self.logger.debug(
                        f"[CONCAT] Date parsing failed for: '{record.date_of_birth}'"
                    )
            return None
        ...
        clean_first_name = self._clean_name_for_concat(
            record.first_name, is_surname=False
        )
    ...
    def process_batch(self, records: List[ClientRecord]) -> List[ClientRecord]:
        """
    ...
    def _load_template(self, file_path: str):
        ...
        try:
            if self.logger:
                ...
                self.logger.info(
                    f"Using column names: ID='{self.template_id_column}', Type='{self.template_type_column}'"
                )
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                fallback_id_col, fallback_type_col = (
                    self._get_consolidated_template_fallback_columns(fieldnames)
                )
            ...
            if self.logger:
                ...
                for txn_ref, data in list(self.template_data.items())[:5]:
                    if sample_count < 3:
                        self.logger.info(
                            f"  Sample: {txn_ref} -> id='{data['id']}', type='{data['type']}'"
                        )
                        sample_count += 1
        except Exception as e:
            if self.logger:
    ...
    def _resolve_template_column(
        ...
        if kind == "id":
            ...
            for norm, original in lower_fields.items():
                if (
                    "identification" in norm or " id " in f" {norm} "
    ...
    def _get_consolidated_template_fallback_columns(
        ...
        fallback_type = self._resolve_optional_template_column(
            fieldnames, type_candidates
        )
    ...
    def _perform_template_validation(self, record: ClientRecord) -> None:
        ...
        if self.logger and record.row_index in [2, 4, 7]:
            self.logger.info(
                f"[TEMPLATE] Row {record.row_index}: txn_ref='{record.transaction_ref}', found={template_entry is not None}"
            )

        if template_entry:
            expected_id = self._normalize_template_value(template_entry.get("id", ""))
            expected_type = self._normalize_template_value(
                template_entry.get("type", "")
            )
            ...
            if record.correction_output:
                if ":" in record.correction_output:
                    correction_id, correction_type = record.correction_output.split(
                        ":", 1
                    )
                else:
            ...
            if self.logger and record.row_index in [2, 4, 7]:
                self.logger.info(
                    f"[TEMPLATE] Row {record.row_index} ({record.transaction_ref}): Raw template data - id='{expected_id}', type='{expected_type}'"
                )
        ...
        if record.correction_output:
            if record.kaizen_error:
                if self.logger and record.row_index in [2, 4, 7]:
                    self.logger.info(
                        f"[TEMPLATE] Row {record.row_index} ({record.transaction_ref}): correction='{record.correction_output}' vs template='{record.kaizen_error}'"
                    )
            ...
            if record.account_type.upper() != "JNT":
                ...
                if len(txn_jnts) == 1:
                    ...
                    if pair_idx is not None:
                        ...
                        aggregated = IDValidationProcessor._aggregate_jnt_pair(
                            aggregated, rec2
                        )
    ...
    def _aggregate_jnt_pair(rec1: ClientRecord, rec2: ClientRecord) -> ClientRecord:
        ...
        rec1.primary_nationality = combine_with_pipe(
            rec1.primary_nationality, rec2.primary_nationality
        )
        rec1.secondary_nationality = combine_with_pipe(
            rec1.secondary_nationality, rec2.secondary_nationality
        )
        ...
        has_correction1 = bool(
            rec1.correction_output and rec1.correction_output.strip()
        )
        has_correction2 = bool(
            rec2.correction_output and rec2.correction_output.strip()
        )
        ...
        if has_correction1 or has_correction2:
            ...
            final_type1 = (
                extract_type_from_correction(rec1.correction_output) or orig_type1
            )
            final_type2 = (
                extract_type_from_correction(rec2.correction_output) or orig_type2
            )

            # Build correction_output showing final state of both clients
            rec1.correction_output = (
                f"{final_id1}|{final_id2}:{final_type1}|{final_type2}"
            )
        ...
        if rec1.kaizen_error or rec2.kaizen_error:
            ...
            if rec1.kaizen_error == rec2.kaizen_error and rec1.kaizen_error:
                ...
                rec1.kaizen_error = combine_with_pipe(
                    rec1.kaizen_error, rec2.kaizen_error
                )
