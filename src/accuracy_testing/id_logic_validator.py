...
class IDLogicValidator:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.last_failure_reason = (
            ""  # Stores reason for most recent validation failure
        )
    ...
    def validate_id_logic(
        ...
        if not id_value or not id_type or id_type.upper() != "NIDN":
            if self.verbose:
                print(
                    f"[LOGIC VALIDATOR] Skipping: id_value={bool(id_value)}, id_type={id_type}, is_NIDN={id_type.upper() == 'NIDN' if id_type else False}"
                )
            self.last_failure_reason = ""
        ...
        if (
            country_code
            and len(id_value) > 2
            and id_value[:2].upper() == country_code.upper()
        ):
            clean_id = id_value[2:]
            if self.verbose:
                print(
                    f"[LOGIC VALIDATOR] Stripped country prefix: '{id_value}' -> '{clean_id}'"
                )

        if self.verbose:
            print(
                f"[LOGIC VALIDATOR] Validating {country_code} NIDN: {clean_id}, DOB={provided_dob}, Gender={provided_gender}"
            )
        ...
        if self.verbose:
            print(
                f"[LOGIC VALIDATOR] No validator for country {country_code}, passing by default"
            )
        return True

    def _log_validation_exception(
        self, country: str, nidn: str, error: Exception, context: str = ""
    ) -> None:
        """
    ...
    def _validate_belgian_nidn(
        self, nidn: str, provided_dob: str, provided_gender: str
    ) -> bool:
        """
    ...
    def _validate_italian_nidn(
        self, fiscal_code: str, provided_dob: str, provided_gender: str
    ) -> bool:
        """
        ...
        try:
            ...
            normalized_gender = (
                provided_gender.strip().upper() if provided_gender else ""
            )
            ...
            # Compare gender
            gender_match = (
                (normalized_gender == extracted_gender)
                if normalized_gender and normalized_gender in ["M", "F"]
                else True
            )
            ...
            if self.verbose and not (dob_match and gender_match):
                ...
                print(
                    f"  Extracted DOB: {extracted_dob}, Provided: {normalized_dob}, Match: {dob_match}"
                )
                print(
                    f"  Extracted Gender: {extracted_gender}, Provided: {normalized_gender}, Match: {gender_match}"
                )

            return dob_match and gender_match

        except Exception as e:
            self._log_validation_exception(
                "IT", fiscal_code, e, "Italian fiscal code validation"
            )
            return True  # Pass with warning logged
        ...
        try:
            ...
            if len(nidn) == 12:
                ...
                if provided_dob:
                    ...
                    year = (
                        1900 + year_2digit if year_2digit >= 50 else 2000 + year_2digit
                    )
            ...
            if not luhn_valid:
                if self.verbose:
                    print(
                        f"Swedish NIDN Luhn check failed on 10-digit format: {nidn_10_digit} (from {nidn})"
                    )
    ...
    def _validate_spanish_nidn(
        ...
        if len(nidn) == 9 and nidn[:8].isdigit() and nidn[8].isalpha():
            ...
            if nidn[8].upper() != expected:
                self.last_failure_reason = f"NIF check letter failed (expected: {expected}, actual: {nidn[8].upper()})"
                return False
            return True
    ...
    def _compare_dob(self, extracted: str, provided: str) -> bool:
        ...
        if extracted != normalized_provided:
            self.last_failure_reason = (
                f"DOB mismatch (ID: {extracted}, Client: {normalized_provided})"
            )
            return False
        return True
    ...
    def _compare_gender(self, extracted: str, provided: str) -> bool:
        ...
        if extracted != normalized_provided:
            self.last_failure_reason = (
                f"Gender mismatch (ID: {extracted}, Client: {normalized_provided})"
            )
            return False
        return True
