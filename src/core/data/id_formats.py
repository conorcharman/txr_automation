...
class IDPattern:
    ...
    def get_mismatch_reason(self, value: str) -> Optional[str]:
        ...
        single_char_classes = len(
            re.findall(r"\[(?:[^\]]+)\](?!\{)", pattern_for_counting)
        )
        ...
        literal_chars = re.findall(
            r"(?<!\[)(?<!\\)(?<![A-Z-])[A-Z](?![A-Z-]|\])", pattern_for_counting
        )
        ...
        if r"^\d" in self.regex_pattern or self.regex_pattern.startswith(r"^\d"):
            if not value.isdigit():
                for i, ch in enumerate(value):
                    if not ch.isdigit():
                        reasons.append(
                            f"Contains non-digit character '{ch}' at position {i+1}"
                        )
                        break
        ...
        if not reasons:
            if expected_len:
                reasons.append(
                    f"Does not match expected {expected_len}-character format"
                )
            else:
...
_RAW_PATTERNS = [
    ...
    (
        "GB",
        "NIDN",
        r"^(?!BG|GB|KN|NK|NT|OO|TN|ZZ)[A-CEGHJ-PR-TW-Z][A-CEGHJ-NP-TW-Z]\d{6}[A-NP-Z]$",
    ),
    ...
    (
        ...
        "NIDN",
        r"^\d{6}\d{3}\d{1}$",
    ),  # 10-digit format: YYMMDDBBBC (without century)
...
class IDFormatManager:
    ...
    def __init__(self):
        ...
        self._patterns_by_key: Dict[Tuple[str, str], List[IDPattern]] = defaultdict(
            list
        )
    ...
    def validate_with_details(
        self, country_code: str, id_type: str, id_value: str
        ...
        if not patterns:
            if id_type.upper() == "CONCAT" and COUNTRY_MANAGER_AVAILABLE:
                ...
                if len(id_value) != 20:
                    return (
                        False,
                        f"Invalid CONCAT format for {country_code}: Expected 20 characters, got {len(id_value)}",
                    )

                if not GENERIC_CONCAT_PATTERN.match(id_value.upper()):
                    return (
                        False,
                        f"Invalid CONCAT format for {country_code}: Does not match pattern [CC][YYYYMMDD][FFFFF][LLLLL] where CC=country, YYYY=year, MM=month, DD=day, F=first name chars, L=last name chars",
                    )
                ...
                if not country_manager.validate_code(prefix):
                    return (
                        False,
                        f"Invalid CONCAT format for {country_code}: Invalid country code prefix '{prefix}'",
                    )
                ...
                return (
                    False,
                    f"No {id_type} format patterns defined for {country_code}",
                )
        ...
        if len(patterns) == 1:
            ...
            return (
                False,
                f"Invalid {id_type} format for {country_code} (tried {len(patterns)} patterns): {reasons[0]}",
            )
