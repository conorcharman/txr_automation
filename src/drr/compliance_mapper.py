...
def _check_report_status(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Report status is missing")
    v = value.strip().upper()
...
def _check_transaction_ref(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Transaction reference number is missing")
    if len(value) > 52:
        return RuleResult.from_ref(
            ref, RuleStatus.FAIL, value=value,
...
def _check_tvtic(value: str | None, ref: RuleReference) -> RuleResult:
    ...
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.NOT_CHECKED, error="Not provided (optional for OTC trades)")
...
def _check_executing_entity(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Executing entity identification code (LEI) is missing")
    ok, err = validate_lei(value)
    status = RuleStatus.PASS if ok else RuleStatus.FAIL
    return RuleResult.from_ref(ref, status, value=value, error=err)


def _check_boolean_field(value: str | None, ref: RuleReference, field_label: str) -> RuleResult:
    """Validate a field that must be 'true' or 'false'."""
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error=f"{field_label} is missing")
    v = value.strip().lower()
...
def _check_trading_capacity(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Trading capacity is missing")
    v = value.strip().upper()
...
def _check_instrument_full_name(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.NOT_CHECKED, error="Not provided — typically sourced from FIRDS")
    if len(value) > 350:
        return RuleResult.from_ref(
            ref, RuleStatus.FAIL, value=value[:50] + "…",
            error=f"Instrument full name exceeds 350-character limit (length={len(value)})",
        )
    return RuleResult.from_ref(ref, RuleStatus.PASS, value=value)


def _check_instrument_classification(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Instrument classification (CFI code) is missing")
    v = value.strip().upper()
...
def _check_notional_currency(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Notional currency 1 is missing")
    v = value.strip().upper()
...
def _check_price_multiplier(value: float | None, ref: RuleReference) -> RuleResult:
    if value is None:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Price multiplier is missing")
    if value <= 0:
        return RuleResult.from_ref(
            ref, RuleStatus.FAIL, value=str(value),
...
def _check_maturity_date(value: str | None, ref: RuleReference) -> RuleResult:
    ...
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.NOT_CHECKED, error="Not provided (only required for debt instruments)")
...
def _check_country_code(value: str | None, ref: RuleReference, field_label: str) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.WARNING, error=f"{field_label} not provided — required where applicable")
    v = value.strip().upper()
...
def _check_execution_within_firm(value: str | None, ref: RuleReference) -> RuleResult:
    if not value:
        return RuleResult.from_ref(ref, RuleStatus.FAIL, error="Execution within firm is missing")
    return RuleResult.from_ref(ref, RuleStatus.PASS, value=value.strip())
...
def check_transaction(
    ...
    # Field 1 — Report status
    report.results.append(_check_report_status(report_status, CATALOGUE["ReportStatus"]))
    # Field 2 — Transaction reference number
    report.results.append(_check_transaction_ref(transaction_ref, CATALOGUE["TransactionReferenceNumber"]))
    # Field 3 — Trading venue transaction identification code (optional)
    report.results.append(_check_tvtic(None, CATALOGUE["TradingVenueTransactionIdentificationCode"]))
    # Field 4 — Executing entity identification code (LEI)
    report.results.append(_check_executing_entity(executing_entity_id, CATALOGUE["ExecutingEntityIdentificationCode"]))
    # Field 5 — Investment firm indicator
    report.results.append(_check_boolean_field(is_investment_firm, CATALOGUE["IsInvestmentFirm"], "Investment firm indicator"))
    # Field 7 — Buyer identification code
    report.results.append(_check_party_id(buyer_id, buyer_id_type, CATALOGUE["BuyerSeller_Buyer"]))
    # Field 16 — Seller identification code
    report.results.append(_check_party_id(seller_id, seller_id_type, CATALOGUE["BuyerSeller_Seller"]))
    # Field 25 — Transmission of order indicator
    report.results.append(_check_boolean_field(transmission_of_order, CATALOGUE["TransmissionOfOrderIndicator"], "Transmission of order indicator"))
    # Field 28 — Trading date time
    report.results.append(_check_datetime(trading_date_time, CATALOGUE["TradingDateTime"]))
    # Field 29 — Trading capacity
    report.results.append(_check_trading_capacity(trading_capacity, CATALOGUE["TradingCapacity"]))
    # Field 30 — Quantity
    report.results.append(_check_quantity(quantity if quantity is not None else net_amount, CATALOGUE["Quantity"]))
    # Field 33 — Price
    ...
    report.results.append(_check_isin(isin, CATALOGUE["InstrumentIdentificationCode"]))
    # Field 42 — Instrument full name
    report.results.append(_check_instrument_full_name(instrument_full_name, CATALOGUE["InstrumentFullName"]))
    # Field 43 — Instrument classification (CFI code)
    report.results.append(_check_instrument_classification(instrument_classification, CATALOGUE["InstrumentClassification"]))
    # Field 44 — Notional currency 1
    report.results.append(_check_notional_currency(notional_currency_1, CATALOGUE["NotionalCurrency1"]))
    # Field 46 — Price multiplier
    report.results.append(_check_price_multiplier(price_multiplier, CATALOGUE["PriceMultiplier"]))
    # Field 54 — Maturity date (debt instruments only)
    report.results.append(_check_maturity_date(maturity_date, CATALOGUE["MaturityDate"]))
    # Field 57 — Investment decision within firm
    report.results.append(_check_investment_decision(investment_decision_maker, CATALOGUE["InvestmentDecisionWithinFirm"]))
    # Field 58 — Country of branch supervising investment decision
    report.results.append(_check_country_code(investment_decision_country, CATALOGUE["PersonResponsibleForInvestmentDecisionCountry"], "Investment decision country"))
    # Field 59 — Execution within firm
    report.results.append(_check_execution_within_firm(execution_within_firm, CATALOGUE["ExecutionWithinFirm"]))
    # Field 60 — Country of branch supervising execution
    report.results.append(_check_country_code(execution_country, CATALOGUE["PersonResponsibleForExecutionCountry"], "Execution country"))
    # Field 65 — Securities financing transaction indicator
    report.results.append(_check_boolean_field(sft_indicator, CATALOGUE["SecuritiesFinancingTransactionIndicator"], "SFT indicator"))
