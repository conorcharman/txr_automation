...
class XLSXConverter:
    ...
    def __init__(
        ...
        if parent_dir and (input_dir or output_dir):
            raise ValueError(
                "Cannot use both parent_dir and input_dir/output_dir modes"
            )
    ...
    def get_output_path(self, xlsx_file: Path) -> Path:
        ...
        if self.parent_dir:
            ...
            if not self.output_dir:
                raise ValueError(
                    "output_dir must be specified in single directory mode"
                )
    ...
    def convert_file_openpyxl(self, xlsx_file: Path, csv_file: Path) -> bool:
        ...
        try:
            ...
            if base_dir:
                ...
                self.logger.info(
                    f"Converting: {xlsx_file.name} ({file_size_mb:.1f} MB)"
                )

            # Warn about large files
            if file_size_mb > 50:
                self.logger.warning(
                    f"  WARNING: Large file detected - this may take several minutes"
                )
            ...
            if csv_file.exists() and not self.force:
                self.logger.warning(
                    f"  WARNING: CSV already exists (use --force to overwrite): {csv_file.name}"
                )
                self.stats.errors += 1
                return False
            ...
            with open(csv_file, "w", newline="", encoding="utf-8-sig") as f:
                ...
                for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
                    ...
                    if row_count % progress_interval == 0:
                        self.logger.info(
                            f"  Processing... {row_count:,} rows read, {output_row_count:,} rows written"
                        )
            ...
            self.logger.info(
                f"  [OK] Created: {csv_file.name} ({row_count:,} input rows -> {output_row_count:,} output rows)"
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="XLSX to CSV Converter - Enhanced with recursive processing",
...
Examples:
  ...
  replay-xlsx-converter --parent-dir C:\\Data\\txr_replay_automation --filter-year FY25 --filter-quarter Q3
  
  # Filter specific phases
  replay-xlsx-converter --parent-dir C:\\Data\\txr_replay_automation --filter-phase phase_ii phase_iii
  
  # Dry run to preview what would be converted
  replay-xlsx-converter --parent-dir C:\\Data\\txr_replay_automation --dry-run
  
  # Original single-directory mode
  replay-xlsx-converter --input-dir ./data/xlsx --output-dir ./data/csv
  
  # Use configuration file
  replay-xlsx-converter --config config/xlsx_converter.yaml
        """,
    )
    ...
    parser.add_argument(
        "--filter-year", type=str, help="Filter by fiscal year (e.g., FY25)"
    )

    parser.add_argument(
        "--filter-quarter", type=str, help="Filter by quarter (e.g., Q3)"
    )
...
def main():
    ...
    # Setup logging
    log_output = config_dict.get("paths", {}).get(
        "log_output", str(parent_dir or output_dir)
    )
    logger = create_logger("xlsx_csv_converter", log_output, args.log_level)

    # Log startup information
    logger.info("=" * 80)
    logger.info("XLSX to CSV Converter - Enhanced Edition")
    logger.info("=" * 80)
