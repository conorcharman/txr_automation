...
from src.accuracy_testing.sql_extract_generator import ExtractBatch, SQLExtractGenerator
...
class TestSQLExtractGenerator:
    ...
    def test_placeholder_detection_alternative_format(self):
        ...
        alt_template = SAMPLE_TEMPLATE.replace(
            "--<<TRANSACTION REFERENCES>>", "--<TRADE REFERENCES>--"
        )
    ...
    def test_write_sql_file_single_batch(self):
        ...
        output_path = generator.write_sql_file(
            output_dir, "test_extract", batch, sql, total_batches=1
        )

        assert output_path.exists()
        assert (
            output_path.name == "test_extract.sql"
        )  # No batch number for single batch
    ...
    def test_write_sql_file_multiple_batches(self):
        ...
        output_dir = self.temp_dir / "output"
        output_path = generator.write_sql_file(
            output_dir, "test_extract", batch, sql, total_batches=3
        )
...
class TestExtractBatch:
    ...
    def test_extract_batch_creation(self):
        ...
        batch = ExtractBatch(batch_number=1, transaction_refs=["REF1", "REF2", "REF3"])
...
class TestValuesMode:
    ...
    def test_split_ref_all_positions(self):
        ...
        k_firm, k_year, k_accl, k_cont, k_suff = (
            SQLExtractGenerator.split_transaction_ref(ref)
        )
        assert k_firm == "ABC"
        assert k_year == "DE"
        assert k_accl == "F"
