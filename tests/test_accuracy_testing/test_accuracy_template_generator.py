...
class TestAccuracyTemplateGenerator:
    ...
    def test_init_with_errors_only(self):
        ...
        generator = AccuracyTemplateGenerator(consolidated_errors=str(self.errors_file))
    ...
    def test_create_template_header_buyer(self):
        ...
        assert len(header) == 16 + 3 + len(SAMPLE_CONSOLIDATED_HEADER)
        assert header[0] == "Transaction Reference"
        assert header[16] == "Agree With Correction"  # First comparison col
    ...
    def test_get_summary(self):
        ...
        )  # 5 original + 1 duplicated from pipe-delimiter
...
class TestTemplateGeneratorIntegration:
    ...
    def test_end_to_end_template_generation(self):
        ...
        with open(buyer_template, "r", encoding="utf-8") as f:
            ...
            # Verify structure
            assert len(header) == 16 + 3 + 5  # validation + comparison + consolidated
            ...
            assert all(row1[i] == "" for i in range(1, 16))

            # Verify consolidated data is appended
            assert row1[19] == "7_37"  # INCIDENT_CODE
            assert row1[22] == "TXN001"  # Transaction reference number
