...
class TestSQLTemplateMapping:
    ...
    def setup_method(self):
        ...
        for template in templates:
            (self.sql_dir / template).write_text(
    ...
    def test_buyer_id_incident_mapping(self):
        ...
        for incident in buyer_incidents:
            ...
            assert (
                result.name == "BuyerID.sql"
            ), f"Incident {incident} should map to BuyerID.sql"
    ...
    def test_seller_id_incident_mapping(self):
        ...
        for incident in seller_incidents:
            result = get_sql_template_for_incident(incident, self.sql_dir)
            assert (
                result.name == "SellerID.sql"
            ), f"Incident {incident} should map to SellerID.sql"
...
OutputFormat=CSV
"""
        (self.sql_templates_dir / "AS400_DataTransfer_template.dtf").write_text(
            dtf_template
        )
    ...
    def create_validated_csv(
        self, fiscal_year: str, quarter: str, incident: str, num_refs: int = 10
    ):
