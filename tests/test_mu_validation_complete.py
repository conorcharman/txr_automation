...
def test_mu_concat_validation():
    ...
    is_valid, error_message = id_format_manager.validate_with_details(
        country_code, id_type, test_id
    )
    ...
    for test_id, cc, description in test_cases:
        ...
        print(
            f"{status} | {test_id:20} | {description:30} | {error if error else 'Valid'}"
        )
