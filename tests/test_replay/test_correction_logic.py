...
class TestIncidentColumnMapper:
    ...
    def test_column_mapping(self):
        ...
        header = ['Transaction Reference', 'Error', 'Correction', 'Correction Field', 
...
class TestCorrectionDecisionLogic:
    ...
    def sample_header(self):
        """Sample CSV header"""
        return ['Transaction Reference', 'Error', 'Correction', 'Correction Field',
                'Agree With Correction', 'Suggested Correction', 'Suggested Correction Field']
    
    def create_test_row(self, txn_ref: str, correction: str, correction_field: str,
                        agree: str, suggested: str, suggested_field: str) -> List[str]:
        """Helper to create a test row"""
        return [txn_ref, 'Y', correction, correction_field, agree, suggested, suggested_field]
    ...
    def test_correction_with_agree_p(self, sample_header, sample_column_config):
        ...
        config = {
            'paths': {'replay_input': 'dummy', 'incident_files': 'dummy',
                     'replay_output': 'dummy', 'log_output': 'dummy'},
            'files': {'replay_patterns': ['*.csv'], 'incident_pattern': 'FY25 Q4 *.csv'},
            'incident_columns': sample_column_config,
            'processor': {'batch_size': 50, 'log_level': 'DEBUG'}
        }
        processor = Phase2Processor(config_dict=config)
    ...
    def test_correction_with_agree_n_and_suggested(self, sample_header, sample_column_config):
        """Test: Correction exists + Agree = 'N' + Suggested exists → Apply Suggested"""
        row = self.create_test_row('TXN004', 'OrigValue', 'Field4', 'N', 
    ...
    def test_correction_with_agree_f_and_suggested(self, sample_header, sample_column_config):
        """Test: Correction exists + Agree = 'F' + Suggested exists → Apply Suggested"""
        row = self.create_test_row('TXN005', 'OrigValue', 'Field5', 'F',
                                   'SuggestedValue', 'SuggestedField')
        mapper = IncidentColumnMapper(sample_header, sample_column_config)
    ...
    def test_correction_with_agree_n_no_suggested(self, sample_header, sample_column_config):
    ...
    def test_correction_with_agree_f_no_suggested(self, sample_header, sample_column_config):
    ...
    def test_no_correction_with_suggested(self, sample_header, sample_column_config):
        ...
        row = self.create_test_row('TXN008', '', '', '', 'SuggestedValue', 'SuggestedField')
        mapper = IncidentColumnMapper(sample_header, sample_column_config)
    ...
    def test_correction_with_unknown_agree_value(self, sample_header, sample_column_config):
    ...
    def test_case_insensitive_agree_values(self, sample_header, sample_column_config):
        ...
        # Test lowercase 'n'
        row_n = self.create_test_row('TXN012', 'OldValue', 'Field12', 'n',
                                     'SuggestedValue', 'SuggestedField')
...
class TestPhase3CorrectionLogic:
    ...
    def sample_header(self):
        ...
        return ['Transaction Reference', 'Error', 'Correction', 'Correction Field',
                'Agree With Correction', 'Suggested Correction', 'Suggested Correction Field',
                'Buyer ID', 'Seller ID', 'Buyer First Name', 'Buyer Last Name',
    ...
    def test_phase3_correction_with_match_type(self, phase3_config, sample_header):
        ...
        row = ['TXN001', 'Y', 'NewValue', 'Field1', 'Y', '', '',
               'BUY123', 'SEL456', 'John', 'Doe', 'Jane', 'Smith']
