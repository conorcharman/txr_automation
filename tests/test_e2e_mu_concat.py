...
def test_e2e_mu_concat():
    ...
    for i, rec_data in enumerate(test_records, 1):
        ...
        print(
            f"  ID Type (before):     '{record.id_type}' (empty={not record.id_type})"
        )
        ...
        print(
            f"  Validation Error:     {result.validation_error if result.validation_error else 'None'}"
        )
        print(
            f"  Correction:           {result.correction if result.correction else 'None'}"
        )
        ...
        if result.prefixed_nationality == "MU":
            ...
            print(
                f"  ✗ Prefixed Nationality NOT set to MU (got: {result.prefixed_nationality})"
            )
