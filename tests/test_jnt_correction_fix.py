...
def test_jnt_both_pass_no_correction():
    ...
    assert (
        len(aggregated) == 1
    ), f"Expected 1 record after aggregation, got {len(aggregated)}"
...
def test_jnt_one_fails_has_correction():
    ...
    assert (
        "AB123456C" in result.correction_output
        or "JK987654L" in result.correction_output
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Joint accounts that pass validation no longer generate false corrections."
        )
