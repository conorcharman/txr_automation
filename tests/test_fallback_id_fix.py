...
def test_fallback_id_detection():
    ...
    print(
        f"✓ Is 'GB_12345' a valid fallback ID for person '12345' in GB? {is_fallback}"
    )
    assert is_fallback, "Failed to detect valid fallback ID"

    # Test invalid cases
    is_fallback_wrong_country = processor.is_fallback_id_pattern(
        "FR_12345", "12345", "GB"
    )
    print(
        f"✓ Is 'FR_12345' a valid fallback ID for person '12345' in GB? {is_fallback_wrong_country}"
    )
    assert (
        not is_fallback_wrong_country
    ), "Should not accept fallback ID with wrong country"

    is_fallback_wrong_person = processor.is_fallback_id_pattern(
        "GB_99999", "12345", "GB"
    )
    print(
        f"✓ Is 'GB_99999' a valid fallback ID for person '12345' in GB? {is_fallback_wrong_person}"
    )
    assert (
        not is_fallback_wrong_person
    ), "Should not accept fallback ID with wrong person code"
...
def test_correction_generation_without_concat():
    ...
    if correction:
        ...
        assert (
            correction_type == "NIDN"
        ), f"Expected NIDN fallback, got {correction_type}"
        assert correction_id == "ES98765", f"Expected ES98765, got {correction_id}"
        print(
            "✓ Correctly generated fallback ID instead of CONCAT for non-CONCAT country"
        )
    else:
