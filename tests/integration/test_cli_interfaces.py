...
class TestCLIInterfaces:
    ...
    def test_phase3_processor_help(self):
        ...
        assert result.returncode == 0
        assert (
            "Phase 3 Processor" in result.stdout or "phase_3_processor" in result.stdout
        )
        assert "--config" in result.stdout
        assert "--use-env" in result.stdout
        assert "--log-level" in result.stdout
    ...
    def test_xlsx_converter_help(self):
        ...
        assert result.returncode == 0
        assert (
            "XLSX to CSV Converter" in result.stdout
            or "xlsx_csv_converter" in result.stdout
        )
        assert "--config" in result.stdout
...
class TestCLIArgumentParsing:
    ...
    def test_xlsx_converter_missing_required_args(self):
        ...
        default_config = (
            PROJECT_ROOT / "config" / "local" / "utils" / "xlsx_converter.yaml"
        )
        if default_config.exists():
            ...
            assert (
                "parent-dir" in result.stdout.lower()
                or "input-dir" in result.stdout.lower()
            )
...
""")
        ...
        assert (
            ...
            or "INVALID_LEVEL" in result.stderr
        )
