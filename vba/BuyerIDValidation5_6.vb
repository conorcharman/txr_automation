...
' ============================================================================
' BUYER ID VALIDATION v5.6 - CORRECTION PRIORITY & SWEDISH LOGIC ENHANCEMENTS
' ============================================================================
' Upgrades in v5.6:
...
' - Added IsSwedishCenturyEligible() and TrySwedishCenturyFix() functions
' Upgrades in v5.5:
' - Updated PerformTemplateLookup to use columns 22 and 23 for ID and ID Type lookup
...
ErrorHandler:
    Call HandleError(Err.Number, Err.Description, "BuyerIDValidation5_6")
    GoTo CleanupAndExit
End Sub
...
Private Sub ProcessSingleClient(ByRef client As ClientRecord, ByRef context As ValidationContext)
    ...
    ElseIf correctTypeFound <> "" Then
        ...
        End If
    Else
        ' No valid format found - try Swedish century logic first
        ...
        ElseIf Not IsCONCATEligible(client, context) Then
            ' CONCAT is not allowed - use fallback
...
Private Function TrySwedishCenturyFix(ByRef client As ClientRecord, ByRef context As ValidationContext) As Boolean
    ...
    ' Check if Swedish century logic applies
    If Not IsSwedishCenturyEligible(client) Then
        Exit Function
    End If
    
    ' Extract century from date of birth (first two digits of year)
    Dim dobYear As String
    dobYear = Format(client.DateOfBirth, "yyyy")
    Dim century As String
    century = Left(dobYear, 2)
    
    ' Create corrected ID by prepending century
    Dim correctedID As String
    correctedID = century & client.OriginalID
    
    ' Test the corrected ID against Swedish NIDN patterns
    If TestIDAgainstAllPatterns(correctedID, client.PriorityCountryCode, "NIDN", context.FormatDict, context.RegexEngine) Then
        ' Validate logic as well
        Dim logicValid As Boolean
        logicValid = ValidateIDLogic(correctedID, "NIDN", client.PriorityCountryCode, Format(client.DateOfBirth, "yyyymmdd"), client.Gender)
...
Private Function IsSwedishCenturyEligible(ByRef client As ClientRecord) As Boolean
    ' Check all conditions for Swedish century logic
    IsSwedishCenturyEligible = (client.PriorityCountryCode = "SE" And _
                               client.OriginalIDType = "NIDN" And _
                               client.OriginalID <> "" And _
                               client.DateOfBirth <> 0 And _
                               Len(client.OriginalID) = 10)  ' Missing century = 10 digits instead of 12
End Function
